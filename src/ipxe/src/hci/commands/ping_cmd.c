...
struct ping_options {
   ...
   /** Inhibit output */
   int quiet;
   /** Message (prefix) to be display after each ping */
   char* msgPrefix;
};
...
static struct option_descriptor ping_opts[] = {
   ...
   OPTION_DESC ( "msgPrefix", 'm', required_argument,
            struct ping_options, msgPrefix, parse_string ),
};
...
static int ping_exec ( int argc, char **argv ) {
   ...
   if ( ( rc = ping ( hostname, opts.timeout, opts.size,
                      opts.count, opts.quiet, opts.msgPrefix ) ) != 0 )
      return rc;
