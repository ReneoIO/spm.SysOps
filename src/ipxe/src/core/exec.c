...
static struct option_descriptor echo_opts[] = {
	...
	OPTION_DESC ( "c", 'c', no_argument,
		      struct echo_options, clearline,  parse_flag ),
	OPTION_DESC ( "i", 'i', no_argument,
		      struct echo_options, indentline, parse_flag ),
};
...
static int echo_exec ( int argc, char **argv ) {
	...
	/* Hack: infer presence of "______" as request to clear line */
	if (strstr(text, "_______") != NULL) {
		char *arg;
                for (arg = text; *arg; arg++) {
			*arg = ' ';
		}
	}
	/* Print text */
	printf ( "%s%s%s%s",
		( opts.clearline  ? "\033[1K\r\033[0K"  : ""   ),
		( opts.indentline ? "          " : ""   ),
		text,
		( opts.no_newline ? ""           : "\n" )
	);
