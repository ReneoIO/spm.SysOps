...
void smp_find_cpus()
{
   ...
   if (fp == NULL) {
     fp = scan_for_floating_ptr_struct((unsigned int *)(639*0x400), 0x400);
   }
   if (fp == NULL) {
     fp = scan_for_floating_ptr_struct((unsigned int *)(0xf0000), 0x10000);
   }
   if (fp == NULL) {
        /* Search the BIOS ESDS area */
        unsigned int address = *(unsigned short *)0x40E;
        address <<= 4;
	if (address) {
     fp = scan_for_floating_ptr_struct((unsigned int *)address, 0x400);
        }
   }
